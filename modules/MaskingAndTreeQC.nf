process maskAln {
    ...
    publishDir { "${params.outdir}/${input_ID}/mask_alignment/" }, mode: "copy"

    input:
    tuple val(input_ID), path(aln_file), path(mask_file)
...
process iqtree {
    ...
    publishDir { "${params.outdir}/${input_ID}/tree/" }, mode: "copy"
...
process treePrune {
    ...
    publishDir { "${params.outdir}/${input_ID}/pruned_tree/" }, mode: "copy"
...
process treeQC {
    ...
    publishDir { "${params.outdir}/${input_ID}/tree-qc/" }, mode: "copy"

    input:
    tuple val(input_ID), path(treefile), path(masked_aln)
    path asr_file
