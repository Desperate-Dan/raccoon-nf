...
workflow tree_qc {
    ...
    if (params.outgroup) {
        ...
        treeQC_outgroup_in = treePrune.out.pruned_tree.join(aln_in)
        treeQC(treeQC_outgroup_in, iqtree.out.asr_file)
    }
    else {
        treeQC_in = iqtree.out.treefile.join(aln_in)
        treeQC(treeQC_in, iqtree.out.asr_file)
    }
}
